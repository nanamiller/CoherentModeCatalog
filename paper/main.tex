...
New clocks for timing experiments:\\ Finding every coherent variable star in \textsl{Kepler}
}
...
\begin{abstract}\noindent
  ...
  The NASA \textsl{Kepler} Mission made precise photometric time-series measurements on  $2\times10^5$ stars.
  Here we search these data for phase-coherent signals that can be used as clocks for precision measurements.
  A signal is phase-coherent, for us, if it does not drift far in angular phase over the 4.1-year Mission lifetime (This sentence is ambiguous).
  We find XXX modes across a total of YYY stars that meet a set of empirical coherence requirements, plus a variability-amplitude cut.
