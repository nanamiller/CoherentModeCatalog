...
\begin{abstract}\noindent
  Precise timings of astrophysical clocks (pulsars and stellar p-modes, for example) have delivered many important discoveries and measurements (gravitational waves and exoplanets, for example). The NASA \textsl{Kepler} Mission made more than $10^{10}$ precise photometric time-series measurements on  $2\times10^5$ stars.
  Here we search these data for phase-coherent signals that can be used as clocks for precision measurements.
  A signal is phase-coherent, for us, if it shows very good clock stability; that is, it does not drift far in oscillation phase over the 4.1-year Mission lifetime.  
  A search of the entire set of \textsl{Kepler} light curves yields thousands of periodic variability signals that meet a set of amplitude, frequency, and empirical coherence requirements.
  These modes are mainly produced by thermal pulsations (as in delta Scuti stars) and eclipsing binaries, but we do not yet have a complete classification of all the signals.
  (*)The stars hosting these modes appear all over the HR diagram, but the coherent pulsators tend to be in or near the instability strip.
  We show a set of examples and deliver a complete catalog. 
\end{abstract}
